<x-layouts.app :title="$supplier->name">
    ...
    <div class="my-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            @can('update', $supplier)
                <a class="rounded-lg bg-surface-muted px-4 py-2 text-white" href="{{ route('suppliers.edit', $supplier) }}">Edit supplier</a>
                <div x-data>
                    ...
                    <dialog x-ref="confirm" class="m-auto w-full max-w-md rounded-xl p-6 backdrop:bg-surface-muted/50" aria-labelledby="supplier-status-title">
                        <h2 id="supplier-status-title" class="text-lg font-semibold">{{ $activating ? 'Activate' : 'Deactivate' }} supplier?</h2>
                        <p class="mt-3 text-muted">{{ $supplier->name }} will become {{ $activating ? 'active' : 'inactive' }}. Existing records will be preserved.</p>
            ...
            @can('delete', $supplier)
                <div x-data>
                    ...
                    <dialog x-ref="confirm" class="m-auto w-full max-w-md rounded-xl p-6 backdrop:bg-surface-muted/50" aria-labelledby="supplier-delete-title">
                        <h2 id="supplier-delete-title" class="text-lg font-semibold">Delete supplier?</h2>
                        <p class="mt-3 text-muted">Permanently delete {{ $supplier->supplier_code }} — {{ $supplier->name }}? This cannot be undone. Referenced suppliers cannot be deleted; deactivate them instead.</p>
    ...
    <dl class="grid gap-6 rounded-xl bg-surface p-6 shadow-sm sm:grid-cols-2">
        @foreach(['Supplier code' => $supplier->supplier_code, 'Name' => $supplier->name, 'Contact person' => $supplier->contact_person, 'Phone' => $supplier->phone, 'Email' => $supplier->email, 'Address' => $supplier->address, 'Status' => ucfirst($supplier->status)] as $label => $value)
            <div><dt class="text-sm text-muted">{{ $label }}</dt><dd class="mt-1 whitespace-pre-wrap break-words font-medium">{{ $value ?? 'Not set' }}</dd></div>
        @endforeach
    </dl>
</x-layouts.app>
