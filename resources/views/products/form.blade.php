<x-layouts.app :title="$product->exists ? 'Edit product' : 'Add product'">
    ...
    @endif
    <form method="POST" action="{{ $product->exists ? route('products.update', $product) : route('products.store') }}" class="max-w-4xl space-y-6 rounded-xl bg-surface p-6 shadow-sm">
        @csrf
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            @foreach(['category_id' => ['Category', $categories], 'brand_id' => ['Brand (optional)', $brands], 'unit_id' => ['Unit', $units]] as $field => [$label, $options])
                <div>
                    ...
                    <select id="{{ $field }}" name="{{ $field }}" @required($field !== 'brand_id') class="w-full rounded-lg border border-border px-3 py-2" aria-invalid="{{ $errors->has($field) ? 'true' : 'false' }}" aria-describedby="{{ $field }}-error">
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full rounded-lg border border-border px-3 py-2" required>
        ...
        <div>
            ...
            <textarea id="description" name="description" maxlength="5000" rows="4" class="w-full rounded-lg border border-border px-3 py-2">{{ old('description', $product->description) }}</textarea>
