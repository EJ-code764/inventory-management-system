<x-layouts.app :title="($record->exists ? 'Edit ' : 'Create ').str($resource)->singular()->title()">
    ...
    <form method="POST" action="{{ $record->exists ? route($resource.'.update', ['record' => $record->id]) : route($resource.'.store') }}" class="max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        ...
        @else
            <div>
                ...
                <textarea id="description" name="description" rows="4" maxlength="5000" class="w-full rounded-lg border border-slate-300 px-3 py-2" aria-describedby="description-error">{{ old('description', $record->description) }}</textarea>
        ...
        <div>
            ...
            <select id="status" name="status" class="w-full rounded-lg border border-slate-300 px-3 py-2" aria-describedby="status-error">
        ...
        <div class="flex items-center gap-4">
            <button class="rounded-lg bg-slate-900 px-4 py-2 font-medium text-white">Save {{ str($resource)->singular() }}</button>
