<form wire:submit="save"
    ...
    <x-alert />
    <p class="text-sm text-slate-600">Enter the new physical stock count, not the amount to add or remove. A reason is
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6">
        ...
        <input id="adjust-search" type="search" wire:model.live.debounce.300ms="search" maxlength="255"
            placeholder="Product, variant, SKU or barcode" class="w-full rounded-lg border border-slate-300 px-3 py-2">
            ...
            @forelse($matches as $item)
                <li wire:key="adjust-choice-{{ $item->id }}" class="flex items-center justify-between gap-4 py-2">
                    <div><x-purchase-item-label :item="$item" /></div><button type="button"
                        ...
                        class="rounded-lg border border-slate-300 px-3 py-1 disabled:opacity-50">Select</button>
            </li>@empty<li class="py-4 text-slate-500">No active products match.</li>
            @endforelse
        </ul>
    </div>
    @if ($currentQuantity !== null && $selectedItem)
        <section class="space-y-5 rounded-xl border border-slate-200 bg-white p-6">
            ...
            <div><span class="mb-1 block text-sm text-slate-500">Product / variant</span><x-purchase-item-label
            ...
            <dl class="grid gap-4 sm:grid-cols-3">
                ...
                <div>
                    <dt class="text-sm text-slate-500">Reserved stock</dt>
            ...
            <div><label for="adjust-new" class="mb-1 block text-sm font-medium">New stock</label><input id="adjust-new"
                    ...
                    aria-describedby="adjust-new-error" class="w-full rounded-lg border border-slate-300 px-3 py-2">
                @error('new_quantity')
    ...
    <div class="flex gap-4"><button type="submit" wire:loading.attr="disabled" @disabled($currentQuantity === null)
            class="rounded-lg bg-slate-900 px-4 py-2 text-white disabled:opacity-50">Record adjustment</button><a
