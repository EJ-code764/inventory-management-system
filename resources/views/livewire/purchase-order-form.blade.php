<form wire:submit="save" class="space-y-6">
    ...
    <div class="grid gap-5 rounded-xl border border-border bg-surface p-6 sm:grid-cols-2">
        <div>
            <label for="purchase-supplier" class="form-label">
                Supplier
            </label>
        ...
        <div>
            <label for="purchase-warehouse" class="form-label">
                Receiving warehouse
            </label>
        ...
        <div>
            <label for="expected-date" class="form-label">
    ...
    <section class="rounded-xl border border-border bg-surface p-6">
        ...
        <label for="stock-search" class="sr-only">
        ...
        <p class="my-2 text-xs text-muted">
            Showing up to 15 active matches. Refine your search to find a specific SKU.
        </p>

        <ul class="max-h-60 divide-y divide-border overflow-y-auto">
            @forelse ($matches as $item)
                <li
                    wire:key="choice-{{ $item->id }}"
                    class="flex items-center justify-between gap-4 py-2"
                >
                    <div>
                        <x-purchase-item-label :item="$item" />
                    </div>
    ...
    <section class="rounded-xl border border-border bg-surface p-6">
        ...
        <p class="mb-3 text-sm text-muted">
            Discount and tax are amounts per line, not percentages.
            Prices and quantities support four decimal places.
            Line subtotals are rounded half-up to four places.
        </p>
            ...
            <table class="w-full text-left text-sm text-foreground">
                ...
                <tbody>
                    @forelse ($form['items'] as $index => $line)
                        <tr
                            wire:key="line-{{ $index }}-{{ $line['stock_item_id'] }}"
                            class="border-b border-border"
                        >
                            <td class="min-w-40 p-2">
                                <x-purchase-item-label
                                    :item="$stockItems->get($line['stock_item_id'])"
                                />
                            </td>
                            ...
                            @foreach ([
                                ...
                                <td class="p-2">
                                    <label
                                        class="sr-only"
                                        for="line-{{ $index }}-{{ $field }}"
                                    >
                                        {{ $label }} for line {{ $index + 1 }}
                                    </label>
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        step="0.0001"
                                        min="{{ $field === 'ordered_quantity' ? '0.0001' : '0' }}"
                                        required
                            ...
                            <td class="p-2 tabular-nums">
                                <x-money
                                    :amount="$totals['items'][$index]['subtotal'] ?? '—'"
                                />
                            </td>
                            ...
                            <td class="p-2">
                                <button
                                    ...
                                    class="font-medium text-red-700 underline dark:text-red-400"
                                >
                                    Remove
                                </button>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-6 text-center text-muted">
                                Add at least one product to begin.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if (!$totals && count($form['items']))
            <p class="mt-3 text-sm text-amber-800 dark:text-amber-300">
                Enter valid quantities and amounts to calculate totals.
                Discounts cannot exceed line subtotals.
            </p>
        @endif
    </section>

    <div>
        <label for="purchase-notes" class="form-label">
            Notes (optional)
        </label>
    ...
    <p class="text-sm text-muted">
        Saving or ordering does not change inventory.
    </p>
</form>