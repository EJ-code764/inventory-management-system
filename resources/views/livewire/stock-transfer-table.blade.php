<div class="space-y-4">
    <div class="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        @foreach (['source' => 'Source warehouse', 'destination' => 'Destination warehouse'] as $field => $label)
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody>
                @forelse($transfers as $transfer)
                    <tr wire:key="transfer-{{ $transfer->id }}" class="border-t">
                        <td class="p-4"><a href="{{ route('stock-transfers.show', $transfer) }}"
                                class="font-medium underline">{{ $transfer->number }}</a></td>
                        <td class="p-4">{{ $transfer->sourceWarehouse->code }}</td>
                        <td class="p-4">{{ $transfer->destinationWarehouse->code }}</td>
                        <td class="p-4">{{ $transfer->transfer_date?->format('Y-m-d') ?? '—' }}</td>
    ...
    {{ $transfers->links() }}<p wire:loading role="status">Loading transfers…</p>
</div>
