<x-layouts.app :title="'Inventory: '.$stockItem->sku">
    ...
    <p class="mt-1 text-muted">{{ $stockItem->sku }} @if($stockItem->variant) · {{ $stockItem->variant->name }} @endif</p>
    <p class="my-4 text-sm text-muted">Default reorder level: {{ $stockItem->reorder_level }}. Warehouse overrides take precedence. Available quantity excludes reservations.</p>
