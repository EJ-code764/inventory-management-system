<x-layouts.app title="Inventory dashboard">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <a class="rounded-lg bg-slate-900 px-4 py-2 text-white" href="{{ route('inventory.index') }}">Stock overview</a>
    </div>
    <p class="mb-4 text-sm text-slate-600">Counts cover all stock items and warehouses, including inactive records and items not yet stocked. Low stock means current quantity is at or below the reorder level.</p>
    <div class="mb-8 grid gap-4 sm:grid-cols-3">
        @foreach(['Item / warehouse pairs' => $pairs, 'Low stock (includes zero)' => $lowStock, 'Out of stock' => $outOfStock] as $label => $count)
            <section class="rounded-xl bg-white p-5 shadow-sm"><h2 class="text-sm text-slate-500">{{ $label }}</h2><p class="mt-2 text-3xl font-semibold">{{ $count }}</p></section>
        @endforeach
    </div>
