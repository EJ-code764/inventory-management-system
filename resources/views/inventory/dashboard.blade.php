<x-layouts.app title="Inventory dashboard">
    ...
    <p class="mb-4 text-sm text-slate-600">Counts cover all stock items and warehouses, including inactive records and
    ...
    <div class="mb-8 grid gap-4 sm:grid-cols-3">
        @foreach (['Item / warehouse pairs' => $pairs, 'Low stock (includes zero)' => $lowStock, 'Out of stock' => $outOfStock] as $label => $count)
            <section class="rounded-xl bg-white p-5 shadow-sm">
                <h2 class="text-sm text-slate-500">{{ $label }}</h2>
