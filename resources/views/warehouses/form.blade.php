<x-layouts.app :title="$warehouse->exists ? 'Edit warehouse' : 'Add warehouse'">
    ...
    <form method="POST" action="{{ $warehouse->exists ? route('warehouses.update', $warehouse) : route('warehouses.store') }}" class="max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        ...
        <div>
            ...
            <textarea id="address" name="address" rows="3" maxlength="255" class="w-full rounded-lg border border-slate-300 px-3 py-2" aria-invalid="{{ $errors->has('address') ? 'true' : 'false' }}" aria-describedby="address-error">{{ old('address', $warehouse->address) }}</textarea>
        ...
        <div class="flex gap-4">
            <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-white">Save warehouse</button>
